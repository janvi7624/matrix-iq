...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ key: string; id: string }> }) {
  ...
  try {
    ...
    const canEdit = viewer.isPrivileged || existing.created_by === viewer.username || isApprover;
    if (!canEdit) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });

    if (body.action === 'approve' || body.action === 'reject') {
      if (!viewer.isPrivileged && !isApprover) return NextResponse.json({ error: 'Forbidden — not the assigned approver' }, { status: 403 });
