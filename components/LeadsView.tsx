...
import { AlertTriangle, Flame, Contact, UserPlus, UserCheck } from 'lucide-react';
...
function LeadsViewContent({ currentUser }: LeadsViewProps) {
  ...
  const [stats, setStats] = useState<{ total: number; today: number; hot: number; unattended: number; unassigned: number; assignedToMe: number } | null>(null);
  ...
  const visibleLeads = useMemo(() => {
    ...
    return sorted;
  }, [leads, q, priorityFilter, interestFilter, unattendedOnly, sortBy, assigneeFilter, currentUser.username]);
  ...
  useEffect(() => {
    setPage(1);
  }, [q, priorityFilter, interestFilter, unattendedOnly, sortBy, assigneeFilter]);
  ...
  const columnCount = canAssign ? 12 : 11;
