...
import { DepartmentRecord, GeneralTaskPriority, GeneralTaskRecord } from '@/lib/types';
...
interface TeamTasksViewProps {
  currentUser: {
    ...
    // Departments THIS viewer manages — empty for privileged viewers, who
    // instead get every department (see manageableDepartments below).
    managedDepartments: { id: string; name: string }[];
  };
}

export default function TeamTasksView({ currentUser }: TeamTasksViewProps) {
  ...
  const canAssign = currentUser.isPrivileged || currentUser.isDepartmentManager;

  // Privileged viewers (admin/superadmin) can assign within any department
  // here, same as every other manager-tier view in this app; a plain
  // department manager only ever sees the department(s) they actually
  // manage — never a cross-department picker (that's Assign Task, the
  // dedicated admin-only tool at /admin/task-assignment).
  const manageableDepartments = useMemo(
    () => (currentUser.isPrivileged ? departments : currentUser.managedDepartments),
  ...
  async function load() {
    ...
    setLoadFailed(false);
    try {
      const [tasksRes, deptRes] = await Promise.all([fetch('/api/team-tasks'), fetch('/api/departments')]);
  ...
  // A manager of exactly one department never gets a choice to make (same
  // as HR Tasks, which locks the field entirely); a manager of several, or a
  // privileged viewer, picks from a real dropdown instead.
  useEffect(() => {
    if (manageableDepartments.length === 1) {
      setForm((f) => (f.departmentId === manageableDepartments[0].id ? f : { ...f, departmentId: manageableDepartments[0].id }));
    }
  ...
  async function handleCreate(e: FormEvent) {
    ...
    try {
      ...
      setForm((f) => ({ ...EMPTY_FORM, departmentId: manageableDepartments.length === 1 ? manageableDepartments[0].id : f.departmentId }));
      await load();
      setTab('mine');
  ...
  if (loading) {
    return (
      <AppShell title="Team Tasks" subtitle="Assign and track tasks within your own department.">
  ...
  if (loadFailed) {
    return (
      <AppShell title="Team Tasks" subtitle="Assign and track tasks within your own department.">
        <ErrorState message="Could not load team tasks — check your connection and try again." onRetry={load} />
      </AppShell>
    );
  }
  ...
  return (
    <AppShell title="Team Tasks" subtitle="Assign and track tasks within your own department.">
      ...
      {tab === 'daily' && (
        dailyTasks.length === 0 ? (
          <EmptyState icon={ClipboardList} title="No tasks due today" message="Team tasks due today will appear here." />
        ) : (
      ...
      {tab === 'assign' && (
        canAssign ? (
            ...
            <FieldRow>
              <Field label="Department">
                {manageableDepartments.length <= 1 ? (
                  <Input value={manageableDepartments[0]?.name || ''} readOnly disabled />
                ) : (
                  <Select
                    value={form.departmentId}
                    onChange={(e) => setForm((f) => ({ ...f, departmentId: e.target.value, assigneeId: '' }))}
                    required
                  >
          ...
          <div className={historyStyles.status}>Only a department manager or admin can assign new Team Tasks. You can still view and work your own tasks.</div>
        )
      )}

      {tab === 'mine' && (
        myTasks.length === 0 ? (
          <EmptyState icon={ClipboardList} title="No team tasks" message="Team tasks in your visibility scope will appear here." />
        ) : (
...
function ActivityTimelineFromTasks({ tasks }: { tasks: GeneralTaskRecord[] }) {
  ...
  return <ActivityTimeline entries={entries} empty="No recent team task activity." />;
}
